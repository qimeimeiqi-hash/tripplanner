...
- **预算不足场景复用现有的超支提示横幅，而非新建一套独立机制**：当用户给的预算不现实地偏低时，
  `buildPrompt` 明确要求 AI 不要为了凑预算而扭曲行程质量，照样给出现实、性价比最高的方案，即使
  真实花费超出用户预算——这必然会触发上面那条超支 20% 的提示横幅，等于自动提醒了"预算不够"，不
  需要再单独设计一套"预算不足"专属提示。
- **出发地/目的地新增"国家/地区"字段用单个自由文本框，而非拆成国家+省份两个字段或国家下拉菜
  单**：全球同名地点（如法国 Paris 和美国得州 Paris）需要靠国家/地区消歧，但完整的全球国家列表
  下拉菜单需要维护一份静态数据、拆两个字段会让表单变长，权衡后选择每侧一个可选的自由文本框（例
  如"日本"或"France, not Texas"），跟 AI 消歧的目的一致又不增加表单复杂度或数据维护成本。
- **预算/天数改为可选后，输入框默认值保持不变（预算 200000、天数 5），而非默认留空**：多数用户
  会直接使用默认值，默认留空反而增加了每次都要手动填写的摩擦；真正想要"AI 自动规划"的用户主动清
  空对应输入框即可触发。
- **网站展示名改为"漫旅 Wanderly"，但仓库名/`package.json` 的 `name`/`vite.config.ts` 的
  `base: '/tripplanner/'` 保持不变**：展示名只是 i18n 里的 `app.title`（`index.html` 的
  `<title>` 同步更新）和 README 标题，纯前端文案层面的改动；仓库名和 `base` 路径被 GitHub Pages
  项目页面地址、Vercel 项目关联、`vite.config.ts` 里的部署判断逻辑等多处依赖，贸然改名会破坏这
  些已经跑通的部署配置（参考 Deployment 一节），收益也不成比例。如果以后真的要连仓库名一起改，
  需要同步走一遍仓库改名+GitHub Pages base path+Vercel 项目重新关联的完整流程，而不是只改一两个
  文件。
- **视觉主题改为抹茶绿 (`--accent: #7a9c68`) + 米白/淡黄背景 (`--bg`)，配手绘风格的花草装饰元
  素，而非极简纯色或大幅插画横幅**：产品定位是"帮用户放松地规划旅行"，柔和的自然色系 + 卡片顶部
  波浪形草地装饰条（`index.css` 里新增的 `--flower`/`--flower-soft` 是给装饰元素用的强调色，不
  是功能色）+ 按钮 hover 时的树叶摇摆动画，比纯色极简风格更贴合"放松"这个产品调性，又不需要额外
  加载任何插画图片资源（全部用内联 SVG/CSS `mask-image` 实现，零新增依赖、零新增网络请求）。
- **生成等待期间的"阶段性文案轮播 + 旅行小贴士轮播 + 树叶浮动插画"三件套，而非显示精确的已用时
  计时器**：目的是让用户在长时间等待时感觉"有进展"而不是感觉"卡住了"，阶段文案（如"正在核算预
  算…"）和小贴士都是编好的固定文案，跟真实生成进度无关，纯粹是缓解焦虑的视觉/文案手段；没有采用
  已用时计时器，因为诚实展示"已经等了很久"反而可能加重焦虑，跟这里想要的效果相反。
- **重试横幅用 `--warning`（琥珀色）而非 `--danger`（红色）**：自动重试是正常的、通常能自愈的
  状态，用报错的红色会让用户误以为已经失败；真正耗尽重试次数后的最终失败横幅仍然是 `--danger`。
- **修改已生成行程的方式，先做"AI 对话式微调"（`buildTweakPrompt`），手动逐项编辑 UI（删除/改
  时间/换景点的直接操作）留到以后**：一句自然语言指令能覆盖"删景点""改时间""换景点"等大多数调
  整场景，且能与"预算超支自动精简"复用同一套机制（自动精简本质上就是一条预先写好的指令，见下一
  条），实现成本远低于一整套所见即所得的行程编辑器。`buildTweakPrompt` 把当前完整 `Itinerary`
  JSON 和用户的自然语言指令一起发给 AI，要求"只改这一处，其余尽量保持原样"，复用跟 `buildPrompt`
  完全相同的 4 个核心模块校验（`parseItineraryResponse`）。微调结果通过 `tripStore.
  updateTripItinerary` 原地更新对应历史记录（不新建一条），`renameTrip` 设置的自定义名字不受影
  响。
- **预算超支的"自动精简方案"按钮复用 AI 微调机制，而非另开一套逻辑**：`buildBudgetTrimInstruction`
  只是按当前语言拼一条"总花费必须控制在 X 预算内"的固定指令，再调用和自由文本微调完全相同的
  `handleTweak` 流程——这样只需要维护一套"改动行程并写回历史"的代码路径。
- **景点卡片新增的开放时间/休息日/参考门票/官方备注都是 AI 估算，非实时数据**：因此这些字段在
  prompt 里明确要求"不确定就不要编造"、允许留空，且只要任意一个景点带了这类信息，前端就会在该板
  块顶部显示一次"仅供参考，请以官方信息为准"的免责声明（`itinerary.highlightInfoDisclaimer`），
  而不是逐条重复标注。
- **出行人数是必填项（默认 2），不像预算/天数那样允许留空交给 AI 猜**：预算/天数 AI 有能力给出
  合理默认值，但出行人数几乎总是用户自己确定知道的信息，没有必要提供"AI 自动决定"的选项；无障碍
  需求（老人/小孩/轮椅）则是可选的多选开关，默认不勾选。
- **地图改为围绕 `highlights`（目的地内的景点）自动缩放定位，而非把 `route`（含出发地）整体纳入
  缩放范围**：出发地和目的地跨洲时，把两端都纳入 `fitBounds` 会导致地图缩得很远、目的地本身反而
  看不清；`MapView` 现在只用 `highlights` 计算缩放边界，并且只绘制落在目的地周边（`highlights`
  外扩 50% 范围内）的路线点连线，出发地这类远处的点不会被画进折线里。
- **历史行程支持重命名（`TripRecord.name`），未命名时仍回退显示"出发地 → 目的地"**：重命名用行
  内表单编辑，而不是浏览器原生 `prompt()` 弹窗——原生弹窗会阻塞页面且无法样式化，也无法被自动化
  工具正常测试。
- **`parseItineraryResponse` 会递归修复被 AI 拆成字符索引对象的字符串字段，而不是只处理已知的
  几个字段**：真实生产报过一次 bug——AI 偶尔会把某个本该是纯字符串的字段（简介、避坑提示、预算
  备注等，位置不固定）拆成 `{"0":"a","1":"b",...}` 这样的字符索引对象，这类对象一旦被直接渲染，
  React 会直接报错崩溃，而这个项目当时没有任何 Error Boundary 兜底，会导致整个页面变白且不报任
  何用户可见的提示。`sanitizeMangledStrings` 在 JSON.parse 之后立刻对整棵响应树做一次递归遍历，
  把"键是连续整数 0..n-1 且值全是字符串"的对象统一识别并拼回原字符串——这样无论 AI 把哪个字段拆
  坏，都能在同一处兜住，而不必对着 summary/note/warning 等每一个文本字段分别写归一化逻辑。
- **`ItineraryView` 外面包了一层 `ErrorBoundary`，`key={result.id}` 而不是内部状态复位**：上面
  那个字符串修复只是"已知的一种"畸形响应，无法保证覆盖所有情况，所以这里作为兜底——任何渲染期间
  抛出的未捕获异常都会被这层边界拦住，展示一条本地化的友好提示而不是让整个页面变白。清除已捕获的
  错误状态用的是换 `key` 强制重新挂载整个组件，而不是在 `componentDidUpdate` 里手动 `setState`
  ——后者是 oxlint 明确警告的模式（容易在没约束好时造成二次渲染甚至死循环），换 `key` 是 React
  官方文档推荐的、更简单可靠的复位方式。
- **所有按钮/标签/chip 统一加"按下微缩小 + 阴影变浅"的点击反馈（`--btn-shadow` 令牌 +
  `:active` 规则），而不是只给单个按钮加加载态**：用户反馈"导出 PDF 点了感觉没反应，直到保存
  窗口弹出来才知道点成功了"——这本质上是"点击那一刻缺乏即时反馈"的通用问题，不是导出 PDF 一个按
  钮的问题，所以在全局层面统一解决，而不是每个按钮各写一套。
- **导出 PDF 按钮额外加了一个持续到导出完成为止的"导出中…"状态，而不是只依赖上面的通用点击反
  馈**：普通按钮点击后动作是瞬时的，但 `exportElementToPdf` 要跑 html2canvas+jsPDF、耗时可达数
  秒，仅有按下瞬间的反馈不够——鼠标一松开、瞬时反馈就消失了，中间这段处理时间用户依然看不出程序
  在跑。这是"点击反馈"和"长耗时操作的进行中状态"两个不同问题，分别解决。
